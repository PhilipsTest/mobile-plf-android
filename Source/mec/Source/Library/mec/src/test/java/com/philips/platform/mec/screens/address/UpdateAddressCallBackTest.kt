...
class UpdateAddressCallBackTest {

    lateinit var  updateAddressCallBack : UpdateAddressCallBack
    @Mock
    lateinit  var addressViewModelMock: AddressViewModel

    @Mock
    lateinit var  isAddressUpdateLiveDataMock : MutableLiveData<Boolean>
    ...
    fun setUp() {
        ...
        addressViewModelMock.isAddressUpdate = isAddressUpdateLiveDataMock

        updateAddressCallBack = UpdateAddressCallBack(addressViewModelMock)

    }

    @Test
    fun `request type should be as expected`() {
        assertEquals(MECRequestType.MEC_UPDATE_ADDRESS ,updateAddressCallBack.mECRequestType)
    }

    @Test
    fun `should assign value to live data on success response comes`() {
        updateAddressCallBack.onResponse(true)
        //TODO
    ...
    fun `should call auth if call auth failure comes`() {
        ...
        updateAddressCallBack.onFailure(errorMock,ecsErrorMock)
    ...
    fun `should update error view model when api fails`() {
        updateAddressCallBack.onFailure(errorMock,null)

...
class ECSCartManagerTest {
    ...
    private fun setApiKey() {
        Mockito.`when`(appConfigurationInterfaceMock.getPropertyForKey(any(String::class.java), any(String::class.java), any(AppConfigurationInterface.AppConfigurationError::class.java)))
                .thenReturn("yaTmSAVqDR4GNwijaJie3aEa3ivy7Czu22BxZwKP")
        Mockito.`when`(appInfraMock.configInterface)
                .thenReturn(appConfigurationInterfaceMock)
    ...
    fun createECSShoppingCart() {
        ...
        Mockito.`when`(requestHandlerMock.handleRequest(CreateCartRequestMock))
                .then { ecscallback.onResponse(eCSShoppingCart) }
    ...
    fun `invalid ctn`() {
        ...
        CreateCartRequestMock = CreateCartRequest("ctn", 1, ecscallback)
        try {
            mECSCartManager.createECSShoppingCart("ct   n", 1, ecscallback) // invalid CTN
        } catch (e: ECSException) {
            assertEquals(ECSErrorType.ECSUnknownIdentifierError.errorCode, e.errorCode)
        }
    ...
    fun `invalid quantity`() {
        ...
        Mockito.`when`(requestHandlerMock.handleRequest(CreateCartRequestMock))
                .then { ecscallback.onResponse(eCSShoppingCart) }
        CreateCartRequestMock = CreateCartRequest("ctn", 0, ecscallback)
        try {
            mECSCartManager.createECSShoppingCart("ctn", 0, ecscallback) // invalid Quantity
        } catch (e: ECSException) {
    ...
    fun `invalid ctn and quantity`() {
        ...
        CreateCartRequestMock = CreateCartRequest("ctn", 0, ecscallback)
        try {
            mECSCartManager.createECSShoppingCart("ct   n", 0, ecscallback) // invalid CTN
        } catch (e: ECSException) {
            assertEquals(ECSErrorType.ECSUnknownIdentifierError.errorCode, e.errorCode)
        }

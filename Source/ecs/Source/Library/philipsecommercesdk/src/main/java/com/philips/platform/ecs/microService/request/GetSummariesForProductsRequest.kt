...
class GetSummariesForProductsRequest(val ecsProducts: List<ECSProduct>, private val ecsCallback: ECSCallback<ECSProducts, ECSError>) : ECSJsonRequest(ecsCallback) {
    ...
    override fun onResponse(response: JSONObject) {
        val ecsProductSummary = response.getData(PRXSummaryListResponse::class.java)
        if (ecsProductSummary?.isSuccess == true) {
    ...
    private fun updateProductsWithSummary(products: List<ECSProduct>, ecsProductSummary: PRXSummaryListResponse) {

...
class CCBRefreshTokenFragment : CCBBaseFragment() {
    ...
    private fun executeRequest() {
        ...
        CCBManager.getCCBSessionHandlerInterface().refreshSession() { success: Boolean, ccbError: CCBError? ->
            if(success){
                progressBar?.visibility = View.GONE
                textView?.text = "Refresh Token Success"
            }else if(ccbError != null){
                progressBar?.visibility = View.GONE
                textView?.text = "Refresh Token Failed"
            }
        }
    }
}
